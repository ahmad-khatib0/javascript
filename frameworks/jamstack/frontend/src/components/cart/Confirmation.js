...
export default function Confirmation({
  ...
  const prices = [
    ...
    },
    {
      ...
      // this called the optional chaining operator. And what this does is it lets us read
      //  the value or check for a value without checking if the chain exists. So this says if
      //  shipping exists, then check for the property price. Otherwise, if it does not exist,
      // then just leave the whole thing as undefined and this will go ahead and prevent the cannot
  ...
  // parseFloat because toFixed function provide a string as the output
  ...
  const handleOrder = async () => {
    ...
    // this is where we'll actually grab the card that we have filled
    //  out so that we can use it to confirm the payment.
    ...
    if (result.error) {
      ...
      axios
        ...
        .catch(error => {
          ...
          console.error(error)
          console.log("FAIL PAYMENT INTENT ", result.paymentIntent.id)
  ...
  useEffect(() => {
    if (!order && cart.length !== 0 && selectedStep === stepNumber) {
      // we don't want to generate a paymentIntent if they cleared the cart
      const storedIntent = localStorage.getItem("intentID")
      const idempotencyKey = uuidv4()

      setClientSecret(null)
      // o make sure that we clear the client secret whenever we're making a new payment intent.
      // so if they go back , and change cart qty , we don't want them to still have the old one
          ...
          process.env.GATSBY_STRAPI_URL + "/orders/process",
          {
            ...
            savedCard:
              user.jwt && user.paymentMethods[cardSlot].last4 !== ""
                ? card.last4
                : undefined,
            // not empty means it must has a saved card on this selected slot
          },
          {
            headers: user.jwt
              ? { Authorization: `Bearer ${user.jwt}` }
  ...
  // we can check if we're actually on the confirmation screen. So if the selected step is equal to the
  //  confirmation step number, in order to prevent making this request at first load of the page ,
  // because now we are rendering all the components at once
    ...
    >
      ...
      <Grid item classes={{ root: classes.buttonWrapper }}>
        <Button
          ...
          // because we need this clientSecret in order to confirm the payment ,
        >
